...
for epoch in range(opt['epochs']):
    ...
    for i, batch in enumerate(train_iter):
        ...
        if (i + 1) % opt['printevery'] == 0:
            ...
            print('epoch: {:03d} - iter: {:04d} - train loss: {:.4f} - time: {:.4f}'.format(epoch, i, avg_loss, time.time()- s))
            total_loss = 0
    ...
    print('epoch: {:03d} - iter: {:04d} - valid loss: {:.4f} - bleu score: {:.4f} - time: {:.4f}'.format(epoch, i, valid_loss, bleuscore, time.time() - s))
